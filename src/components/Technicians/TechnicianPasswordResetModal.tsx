...
interface TechnicianPasswordResetModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function TechnicianPasswordResetModal({ 
  isOpen, 
  onClose, 
  onSubmit, 
  technicianName,
  technicianEmail 
}: TechnicianPasswordResetModalProps) {
  const [loading, setLoading] = useState(false);

  const handleSubmit = async () => {
    setLoading(true);
    try {
      await onSubmit(technicianEmail);
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg w-full max-w-md mx-4">
        ...
        <div className="p-6">
          <p className="text-sm text-warm-600 mb-4">
            Envoyer un email de réinitialisation de mot de passe à ce technicien.
          </p>
          ...
          <div className="bg-creho-50 border border-creho-200 rounded-lg p-4 mb-6">
            <div className="flex items-center">
              ...
              <div>
                <p className="font-semibold text-creho-900">{technicianName}</p>
                <p className="text-sm text-creho-700">Un email sera envoyé à:</p>
                <p className="text-sm text-creho-800 font-medium">{technicianEmail}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-sm text-blue-700">
              Le technicien recevra un email avec un lien pour définir un nouveau mot de passe. 
